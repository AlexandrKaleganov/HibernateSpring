...
/**
 * Класс диспетчер комманд, используется для выполняния команд,
 * которые приходят в запросе.
 * в хешмапе содержаться 2 команды (два ключа)
 * findById  и  list, значения по ключу реализованы
 * с помощью функционального интерфейса Function
